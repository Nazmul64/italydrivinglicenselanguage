                            ...
                            @foreach($ch->pages as $index => $page)
                                ...
                                <div class="chapter-image-card scheda-item-card" data-cartelli-page-id="{{ $page->id }}" data-chapter-id="{{ $ch->id }}" onclick="handleCartelliSchedaCardClick({{ $ch->id }}, {{ $page->id }})">
                                    <div style="display: flex; flex-direction: column; align-items: center; height: 100%; justify-content: space-between; width: 100%; position: relative;">
                                        ...
                                        @if(!empty($pageImage))
                                        <div class="chapter-card-img-wrapper" style="width: 100%; height: 230px; min-height: 180px; display: flex; align-items: center; justify-content: center; margin: 10px auto; background: transparent; overflow: hidden; border-radius: 14px; padding: 0;">
                                            <img src="{{ $pageImage }}" onerror="this.parentElement.style.display='none'" class="chapter-card-img" alt="{{ $displayTitle }}" style="height: 100%; width: 100%; max-height: 230px; object-fit: contain; border-radius: 14px; background: transparent; display: block;">
                                        </div>
                                        @endif
...
<script>
    ...
    function openCartelliPageScreen(chId, pageId) {
        ...
        if (targetBox) {
            ...
            syncCartelliBookmarkStates(targetBox);
    ...
    function syncCartelliBookmarkStates(container) {
        const root = container || document;
        const cBookmarks = JSON.parse(localStorage.getItem('cartelli_bookmarks') || '[]');
        root.querySelectorAll('[id^="cartelli-mcq-card-"]').forEach(card => {
            const match = card.id.match(/\d+/);
            if (!match) return;
            const qId = parseInt(match[0]);
            let isSaved = cBookmarks.includes(qId) || cBookmarks.includes(String(qId));
